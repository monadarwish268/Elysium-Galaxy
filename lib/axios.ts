...
})   // l api hiye instance mn axios fa kermel ma aatu
...
function toApiError(error: unknown, fallback: string): ApiError {
    if (axios.isAxiosError(error)) {
        ...
        const axiosError = error as unknown as { message?: string }
        const message = body?.message ?? body?.error ?? axiosError.message ?? fallback

...
export default function HappyPlanetPage() {
  const planet = PLANETS_DATA.happy;
  ...
  const getPlaceIcon = (type: string) => {
    if (type.includes('Bookstore') || type.includes('Café')) return <Book className="w-4 h-4 text-yellow-400" />;
    if (type.includes('Meditation')) return <HeartHandshake className="w-4 h-4 text-yellow-400" />;
    return <Trees className="w-4 h-4 text-yellow-400" />;
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-[#0b132b]/50 border border-slate-800/80 rounded-2xl min-h-[220px] relative overflow-hidden flex flex-col justify-between backdrop-blur-md">
              ...
              <div className="p-3 bg-slate-950/90 text-slate-400 text-[11px] flex justify-between items-center border-t border-slate-800">
                <span className="flex items-center gap-1 text-yellow-400"><MapPin className="w-3.5 h-3.5" /> Live mood map</span>
                <span>{planet.places.length} matches near Beirut</span>
              </div>
            </div>

            <div className="space-y-3">
              {planet.places.map((place, idx) => (
                <div key={idx} className="bg-[#0b132b]/50 border border-slate-800/80 p-4 rounded-xl flex items-center gap-4 backdrop-blur-md hover:border-yellow-500/40 transition">
                  <div className="w-9 h-9 rounded-lg bg-yellow-950/60 border border-yellow-800/40 flex items-center justify-center shrink-0">

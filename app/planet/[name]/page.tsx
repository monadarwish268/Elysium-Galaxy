...
import React, { useState } from 'react';
import { useParams, useRouter } from 'next/navigation';
import { ArrowLeft, MapPin, Book, Trees, Coffee, Flower2, Plus } from 'lucide-react';
import { PLANETS_DATA } from '../../../data/planetsData';
import VideoCard from '../../components/VideoCard';
import ReflectionCard from '../../components/ReflectionCard';
import ActivityCard from '@/components/ActivityCard';

// 1. استيراد الـ Hooks الديناميكية للأنشطة
import {
...
// أماكن مخصصة لطرابلس، لبنان بحسب المود (Static كما هي)
const TRIPOLI_PLACES: Record<string, Array<{ name: string; type: string; distance: string; desc: string; coords: string }>> = {
  calm: [
    { name: 'Ahwek Café - Mina', type: 'Quiet Café', distance: '1.2 km', desc: 'Cozy seaside spot in Al-Mina for relaxation', coords: 'Al-Mina Corniche' },
    { name: 'Mina Seaside Corniche', type: 'Nature & Walk', distance: '2.0 km', desc: 'Calm waves and refreshing sea breeze', coords: 'Al-Mina' },
...
export default function DynamicPlanetPage() {
  ...
  // بيانات الكوكب (يمكنك استبدال PLANETS_DATA بجلب الكوكب من DB إذا كان لديك Hook للكواكب)
  const planet = PLANETS_DATA[planetKey] || PLANETS_DATA.calm;
  const tripoliPlaces = TRIPOLI_PLACES[planetKey] || TRIPOLI_PLACES.calm;

  // 2. جلب الأنشطة ديناميكياً من قاعدة البيانات للـ Planet الحالي
  const { data: dbActivities, isLoading: isLoadingActivities } = useGetActivitiesQuery();
  const createActivityMutation = useCreateActivityMutation();
  const deleteActivityMutation = useDeleteActivityMutation();

  // تصفية الأنشطة القادمة من الـ Database لتأخذ فقط الأنشطة الخاصة بهاد الكوكب (planetId)
  const planetActivities = dbActivities?.data?.filter(
    (act) => act.planetId === name
  ) || [];
  ...
  return (
    <div className="min-h-screen bg-transparent text-slate-100 p-6 md:p-12">
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <button
          onClick={() => router.push('/galaxy1')}
          className="flex items-center gap-2 text-xs text-slate-400 hover:text-white transition bg-slate-900/60 border border-slate-800 px-4 py-2 rounded-full w-fit cursor-pointer"
        >
          <ArrowLeft className="w-4 h-4" />
          Back to galaxy
        </button>
        ...
        {/* Dynamic Personalized Wellness Activities */}
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold text-white">Personalized wellness activities</h2>
            <span className="text-sky-400 text-xs bg-sky-950/10 border backdrop-blur-[20px] border-sky-800/30 px-3 py-1 rounded-full font-medium">
              {completedActivities.length} of {planetActivities.length} complete
            </span>
          </div>

          {/* حالة التحميل Loading */}
          {isLoadingActivities ? (
            <div className="text-slate-400 text-sm py-8 text-center">Loading activities from database...</div>
          ) : planetActivities.length === 0 ? (
            <div className="text-slate-500 text-sm py-6 text-center border border-dashed border-slate-800 rounded-2xl">
              No activities found for this planet in the database.
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {planetActivities.map((act) => (
                <ActivityCard
                  key={act.id}
                  activity={{ ...act, duration: '5 min' }}
                  isCompleted={completedActivities.includes(act.id)}
                  onToggle={toggleActivity}
                // onDelete={() => deleteActivityMutation.mutate(act.id)} // إمكانية الحذف المباشر
                />
              ))}
            </div>
          )}
        ...
        {/* Licensed Psychologist Recommendations (Static) */}
        <div className="space-y-4">
          <div>
            <h2 className="text-2xl font-bold text-white">Licensed psychologist recommendations</h2>
            <p className="text-slate-400 text-xs mt-1">Short, evidence-based sessions curated for this planet.</p>
        ...
        {/* AI Places Nearby - Tripoli Lebanon (Static) */}
        <div className="space-y-4">
          <div>
            <h2 className="text-2xl font-bold text-white">AI mood-based places in Tripoli, Lebanon</h2>
            <p className="text-slate-400 text-xs mt-1">Real spots in Tripoli chosen to match your current vibe.</p>
